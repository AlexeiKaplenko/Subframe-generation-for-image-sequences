...
def _pil_loader(path, train=True, cropArea=None, resizeDim=None, frameFlip=0):
    """
    ...
    with open(path, 'rb') as f:
        ...
        if train:
            # Gaussian blur
            blurred_img = cropped_img.filter(ImageFilter.GaussianBlur(radius=1.6))
...
class SuperSloMo(data.Dataset):
    ...
    def __init__(self, root, transform=None, dim=(6144, 4096), randomCropSize=(640, 640), train=True):
        """
        Parameters
        ----------
            root : string
            ...
            dim : tuple, optional
                Dimensions of images in dataset. Default: (640, 640)
            randomCropSize : tuple, optional
                Dimensions of random crop to be applied. Default: (640, 640)
    ...
    def __getitem__(self, index):
        ...
        if (self.train):
            ### Data Augmentation ###
            # To select random 5 frames from all frames in a clip
            firstFrame = random.randint(0, len(self.framesPath[index])-9)
            ...
            #frameRange = range(firstFrame, firstFrame + 5) if (random.randint(0, 1)) else range(firstFrame + 4, firstFrame - 1, -1)
...
#             firstFrame = 0 #!!!
        ...
        for frameIndex in frameRange:
            ...
            image = _pil_loader(self.framesPath[index][frameIndex], train=True, resizeDim = None , cropArea=cropArea, frameFlip=randomFrameFlip)
...
class Video(data.Dataset):
    ...
    def __init__(self, root, transform=None):
        ...
        frame        = _pil_loader(framesPath[0], train=False)
    ...
    def __getitem__(self, index):
        ...
        for framePath in [self.framesPath[index], self.framesPath[index + 1]]:
            ...
            image = _pil_loader(framePath, train=False, resizeDim=None)
            # Apply transformation if specified.
            if self.transform is not None:
                image = self.transform(image)
            sample.append(image)
